        ...
        switch (text){
            case "/start": {
                if (!dataManager.userIsView(userId)) {
                    logger.info("Клиент:  ( тг_имя: "+ user_first_name +", тг_ф.: "+user_last_name+", тг_Id: "+ userId+"  тг_ник: " +dataManager.getUserData(userId) +"  :  /start");
            ...
            case "Услуги и цены": {
                logger.info("Клиент:  ( тг_имя: "+ user_first_name +", тг_ф.: "+user_last_name+", тг_Id: "+ userId+"  тг_ник: " +dataManager.getUserData(userId) +"  :   Смотрит раздел 'Услуги и цены'");
            ...
            case "Напомнить время записи": {
                logger.info("Клиент:  ( тг_имя: "+ user_first_name +", тг_ф.: "+user_last_name+", тг_Id: "+ userId+"  тг_ник: " +dataManager.getUserData(userId) +"  :  Напоминание времени записи");
            ...
            case "Записаться на маникюр": {
                if (dataManager.getAdmin().isAppointments()) {
                    if (dataManager.customerManicureRegStatus(userId) == DataManager.manicureRegStatus.NO_REG_ERROR) {
                        logger.info("Клиент:  ( тг_имя: "+ user_first_name +", тг_ф.: "+user_last_name+", тг_Id: "+ userId+"  тг_ник: " +dataManager.getUserData(userId) +"  :  Хочет записаться на маникюр");
            ...
            case "Посмотреть портфолио": {
                if (dataManager.getServices().size() != 0) {
                    logger.info("Клиент:  ( тг_имя: "+ user_first_name +", тг_ф.: "+user_last_name+", тг_Id: "+ userId+"  тг_ник: " +dataManager.getUserData(userId) +"  :  Просмотр портфолио");
            ...
            case "Отменить запись": {
                logger.info("Клиент:  ( тг_имя: "+ user_first_name +", тг_ф.: "+user_last_name+", тг_Id: "+ userId+"  тг_ник: " +dataManager.getUserData(userId) +"  :  Хочет отменить запись на маникюр");
            ...
            case "Закончить" : {
                if (dataManager.userActivityStatus(userId)){
                    dataManager.userSetDefault(userId);
                    logger.info("Клиент:  ( тг_имя: "+ user_first_name +", тг_ф.: "+user_last_name+", тг_Id: "+ userId+"  тг_ник: " +dataManager.getUserData(userId) +"  :  Отмена действия");
                    return List.of(KeyboardsManager.getUserKeyboard("Здравствуйте, " + dataManager.getUserName(userId) + "! Что бы Вы хотели?", update));
                }
            }
            ...
            default: {
                if (dataManager.userIsView(userId) && dataManager.getServicesByName().contains(text)) {
                    logger.info("Клиент:  ( тг_имя: "+ user_first_name +", тг_ф.: "+user_last_name+", тг_Id: "+ userId+"  тг_ник: " +dataManager.getUserData(userId) +"  :  Просматривает галерею портфолио '"+text+"'");
