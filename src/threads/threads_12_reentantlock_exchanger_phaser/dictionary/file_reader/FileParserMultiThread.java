...
public class FileParserMultiThread implements Runnable {
    ...
    public FileParserMultiThread(String startFrom, String readableFile, long startPos) {
        ...
        if (createNewFile) {
            String file = new File(readableFile).getParent() + "\\" + "result_new_multi.txt";
                ...
                if (!threadMap.containsKey(strS)) {
                    try {
                        ...
                        if (!threadMap.containsKey(strS) && Character.isAlphabetic(c)) {
                            Thread newReader = new Thread(new FileParserMultiThread(strS, readableFile, startPos));
                            newReader.start();
    ...
    private void writeResultInFile() {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(new File(readableFile).getParent() + "\\" + "result_new_multi.txt", true))) {
